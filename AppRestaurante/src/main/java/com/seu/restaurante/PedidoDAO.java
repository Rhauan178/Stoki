...
public class PedidoDAO {
    ...
    public void salvarPedido(Pedido pedido) {
        String sqlPedido = "INSERT INTO pedidos(id_mesa, status, data_hora, observacao) VALUES(?,?,?,?)";
        ...
        try (Connection conn = DatabaseConnector.connect()) {
            ...
            try (PreparedStatement pstmtPedido = conn.prepareStatement(sqlPedido, Statement.RETURN_GENERATED_KEYS)) {
                ...
                pstmtPedido.setString(4, pedido.getObservacao());
    ...
    // --- MÉTODOS DE CONVENIÊNCIA (SEM FILTRO) QUE ESTAVAM EM FALTA ---
    public List<Pedido> carregarPedidosAtivos() { return carregarPedidosAtivos(null, null); }
    public List<Pedido> carregarPedidosPagos() { return carregarPedidosPagos(null, null); }
    public List<Pedido> carregarPedidosArquivados() { return carregarPedidosArquivados(null, null); }

    // --- MÉTODOS DE CARREGAMENTO COM FILTRO ---
    public List<Pedido> carregarPedidosAtivos(LocalDate dataInicio, LocalDate dataFim) {
        String sql = "SELECT * FROM pedidos WHERE status != 'PAGO' AND status != 'ARQUIVADO'";
        return carregarPedidos(sql, dataInicio, dataFim);
    }
    public List<Pedido> carregarPedidosPagos(LocalDate dataInicio, LocalDate dataFim) {
        String sql = "SELECT * FROM pedidos WHERE status = 'PAGO'";
        return carregarPedidos(sql, dataInicio, dataFim);
    }
    public List<Pedido> carregarPedidosArquivados(LocalDate dataInicio, LocalDate dataFim) {
        String sql = "SELECT * FROM pedidos WHERE status = 'ARQUIVADO'";
        return carregarPedidos(sql, dataInicio, dataFim);
    }

    private List<Pedido> carregarPedidos(String baseSql, LocalDate dataInicio, LocalDate dataFim) {
